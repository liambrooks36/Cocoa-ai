...
def macro():
    ...
    if (
        ...
        and now - MACRO_CACHE["timestamp"] < MACRO_CACHE_SECONDS
    ):
...
def news():
    ...
    headers = {
        "User-Agent": "Mozilla/5.0 (compatible; SilverAI/1.0.1)",
    }
...
def save_prediction(payload: dict):
    ...
    macro_text = str(
        payload.get("macro", payload.get("macro_score", "")) or ""
    )
...
Use the supporting markets as context, never as a substitute for the silver trigger:

- Gold (GC=F): agreement with gold can strengthen a precious-metals move.
- US Dollar Index (DXY): a stronger dollar is commonly a headwind for dollar-priced silver;
  a weaker dollar can be supportive. Do not treat this as an absolute rule.
...
Prioritise fresh market-moving headlines involving:
silver, gold, the US dollar, Federal Reserve policy, CPI/PCE/inflation,
jobs data, Treasury yields, geopolitical shocks and major industrial-demand news.

Old or generic silver-mining company stories should carry little weight for an immediate scalp.

MACRO SCORE

macro_score is:
-10 = macro backdrop strongly bearish for silver now
0 = mixed / neutral
+10 = macro backdrop strongly bullish for silver now

Do not force a macro score simply because one correlated market moved.
